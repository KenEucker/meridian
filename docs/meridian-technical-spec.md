...
- Every surface the deployment serves for an organization resolves under its subdomain: the public surfaces (today the event application flow), the organization-branded signed-in clients, and the `/api` endpoints they call — so an organization lives entirely on its subdomain, which is the same shape the dedicated-infrastructure tier later serves in isolation.
- The platform domain is derived from the configured application URL's host; no separate configuration key exists. A request host of the form `<organization-slug>.<application host>` is organization addressing, and any other host falls through to path resolution. Development derives the same way from an application URL on `localhost`, giving `<slug>.localhost`.
- No subdomain labels are reserved. Any active organization slug resolves, and operational hostnames (`www`, `api`, `mail`, and the like) are the deployment's own DNS concern rather than the application's.
...
- A subdomain that matches no active organization slug is a not-found response. An archived organization's slug is not an active slug: archiving takes the organization's subdomain and its root path out of resolution.
...
37. Exact years-of-service calculation. Requirements 2.8 lists it among the staff history Meridian preserves and no document defines how it is derived — from first organization status, from first event worked, from first shift with recorded hours, counted in whole years or elapsed years, and what a gap in participation does to it. The Directory person entry (DIR-029) is the first surface that renders it. Settled with the product owner in M18.73: the count of distinct calendar years in which the person has at least one shift with recorded hours in the organization's events. A gap year does not count, a person with no recorded hours reads zero, and the count is against the organization whichever context is resolved, because service history belongs to the organization rather than to the event being looked at.
38. Exact tie-break for the closest applicable occurrence when a Directory search result matches a person who holds several equally applicable chart locations (DIR-035). Every authorized occurrence is highlighted regardless; this decides only which one the surface scrolls to. Settled with the product owner in M18.76: each search result row already names one authorized location as its breadcrumb (DIR-034), so selecting a row scrolls to that row's own chart node; where a tie somehow remains, the surface scrolls to the first authorized occurrence in chart order — the Organizers Department first, then departments as drawn.
