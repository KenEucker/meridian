...
**Goal:** Produce versioned, installable Alpha 1 builds of all three products, make organizations reachable at organization subdomains beside their root paths, let an organization run only the modules it needs, and verify release readiness.

**Primary source docs:** Technical spec sections 3.2 through 3.4, 8, 15A, 25, 26, 27, 28, 29; requirements 7.1 (ORG-018, ORG-022 through ORG-025), 7.27 (MOD-001 through MOD-022); data/API spec 5.9, 6.8, 7.6, 10.1A; process release checklist; `docs/process/versioning-strategy.md`; QA README; UI/kiosk docs.
...
- packaging the server as anything but the M19.2 deployment bundle and image. There is no desktop-managed server: technical spec 25.1 keeps server process management out of the Electron wrapper for Alpha 1.
