...
| M18.63 On-site sign-in QA script | Add `QA-AUTH-02-on-site-device-sign-in.md`, covering scan-to-sign-in, the `short_code` fallback, the untargeted code, foreign-node refusal, request expiry and re-open, and scan re-authentication. QA-AUTH-01 exercises only the God Mode console path, which is why none of this has been checkable by hand. | QA README | Human QA script |
