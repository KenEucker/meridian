...
- `readiness`: what the set is bounded by — when it was composed, the context event and the window its staleness is bounded by (11A.4), the node's event lock, the effective role codes and active modules it was composed under, the sections this build cannot compose yet, the freshness of any computed section, and a row count per section
...
Sections come in two kinds. Every staff member receives the regular-staff list of technical spec 9.3, scoped by their own memberships — including `event_info`, the node's own assembled and rendered Event Info per event (11.4A, POL-022), carried whole so a device never runs a second document renderer. A role adds further sections, scoped by the grants that carry it: department Logistics, department Operations, department Planning, designated shift lead, department lead, and event-scoped Incident Command. A role-additive section is absent for a caller who does not hold its role, rather than present and empty — an empty department roster is a claim about that department, and a caller with no standing in it is not entitled to make one.

Role scope is resolved per event rather than per caller. An event-scoped grant composes sections for its own event only; an unscoped grant composes them for every event the caller reaches. The `shift_lead` designation narrowing (TEAM-009) and the Incident Command department check apply here exactly as they apply to any other read, because it is the same resolver answering.

One section kind is computed rather than stored: the Planning Table's identity-free plan-versus-actual aggregates (SLB-019). A device cannot recompute them, because they are counts of identities it is deliberately not given, so `readiness.aggregate_freshness` reports when the node computed each one. It is reported there rather than on the rows because a timestamp inside a section would enter the version and make every refresh a transfer.

An Incident Command grant (`ic_lead`, `ic_operator`, `ic_viewer`) composes `ims_incidents`: a bounded preload of the event's open incidents and its most recently created entries, capped (technical spec 9.3 as amended 2026-08-20), serialized with their timelines by the same serializer the 5.1 incident reads use. The bound is the "not greedily synced" rule stated as caps rather than as absence; the whole incident history never travels, and a caller without IC standing at the event receives no incident section at all. The event map package keeps its own permission rules and is not part of this endpoint.
