...
Sections come in two kinds. Every staff member receives the regular-staff list of technical spec 9.3, scoped by their own memberships. A role adds further sections, scoped by the grants that carry it: department Logistics, department Operations, department Planning, designated shift lead, and department lead. A role-additive section is absent for a caller who does not hold its role, rather than present and empty — an empty department roster is a claim about that department, and a caller with no standing in it is not entitled to make one.
...
Incidents are not composed into the set at any role (technical spec 9.3). The event map package and the Incident Command cache list have their own permission rules and are not part of this endpoint.
