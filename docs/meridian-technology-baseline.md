...
| Client app framework | Vue 3.x | `vue:^3.5`, locked by pnpm | Shared web/desktop/mobile product client framework. Do not introduce a competing SPA framework. |
| Client app router | Vue Router 4.x | `vue-router:^4.5`, locked by pnpm | Client-side routing for the shared Vue client. |
| Frontend language | TypeScript 5.x | `typescript:^5.7`, locked by pnpm | Used for the shared Vue client and generated clients. Type-check Vue single-file components with `vue-tsc`. |
...
| JavaScript/Vue testing | Vitest 4.x | `vitest:^4.1`, locked by pnpm | Vite-native unit/component test runner for the shared Vue client. Use with `@vue/test-utils:^2.4` and a `jsdom` environment. |
...
| Reverse proxy / TLS | Caddy 2.x | `caddy:2-alpine` in the deployment bundle; pin by major/minor or digest | Named by the technical specification rather than chosen here: section 5.1 lists "Caddy or equivalent reverse proxy" and section 4 places its configuration at `deploy/caddy`. Terminates TLS in front of the Laravel server and proxies PHP over FastCGI. Automatic ACME issuance serves an internet-reachable node; an event node serves a certificate provisioned before the event, because a field network cannot answer an ACME challenge (technical spec 8.3). Recorded in this table by M19.2, which is the task that first ships a real configuration for it. |
| Server PHP runtime image | `php:8.5-fpm-alpine` | Track the project's PHP baseline; pin major/minor | The deployment bundle's server image. php-fpm rather than a bundled web server because Caddy already terminates TLS and serves the public root, and rather than an all-in-one PHP server binary because that would be a second runtime to approve. Extensions compiled in: `gd` (photo and profile-picture resizing), `zip` (spreadsheet imports), `pdo_pgsql`, `bcmath`, and `pcntl` (queue worker signals). Zend OPcache ships with the base image and is configured rather than installed. |
