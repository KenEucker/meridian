...
- A build packaged with `corepack pnpm run release:package`, or at minimum `corepack pnpm run docs:package` and `corepack pnpm run changelog:generate`, so operator documentation and the changelog data file are present.
...
- Organization **Idaho Burners** with at least one active department, but **no** Organizers Department and **no** default Incident Command Department set.
- Organization **Empty Collective** with **no** departments at all.
- Event **Idaho Decompression 2026** under Idaho Burners with **no** assigned departments and no Incident Command override.
- No staff member holding the Lead Organizer role through an active team membership in Idaho Burners.
...
5. **Read the configuration group.** Confirm it reports the state of this node — for a node that is not paired, an item naming the incomplete pairing. On a node running in event mode without trusted HTTPS or with PowerSync down, confirm those are reported too. Confirm no item displays a secret value: a missing key is reported as missing, never quoted.
6. **Read the data gap group.** Confirm each seeded gap appears once: *Empty Collective has no departments*; *Idaho Burners has no Organizers Department*; *Idaho Burners has no default Incident Command Department*; *Idaho Burners has no active Lead Organizer*; *Idaho Decompression 2026 has no assigned departments*; *Idaho Decompression 2026 has no resolvable Incident Command Department*. Confirm **Empty Collective** reports only the missing-departments item — the downstream gaps are not piled on top of it.
...
- Documentation renders the packaged operator tree with no network access, filters by title and heading, shows documentation version against build version, and serves nothing outside `docs/operator/`.
