...
`REPORT-014` asks for, which M18.26 built and the product titles Exports:
`organizer.exports` for the organization/event scope and `department.exports`
for the department scope. The
...
- `REPORT-014`: the organizer and department export surfaces, each offering only the exports the actor may run and stating the scope and the excluded fields before generation.
...
29. Sign out, sign in as Dana Departmentlead, and open Home. Look for Exports
    under Organization pages, then open **Exports** under Department pages.
30. Run the shift roster export from Dana's page and open the file. Then edit
    ...
    id}/exports` and read what the page offers.
31. Sign in as Ira Ineligible and look for either Exports entry on Home. Then
    open `/organizer/exports` by address.
32. Back as Dana, disconnect the device from the network — airplane mode, or
    stopping `php artisan serve` — reload the department Exports page, and try
    to run an export.
...
- Step 25: Home lists **Exports** under Organization pages and lists no Exports entry under Department pages. The page names the event and the role it was reached by.
...
- Step 29: Dana's Home lists **Exports** under Department pages and lists none under Organization pages. Her page names Rangers and states that every export from it is scoped to Rangers.
...
- Step 31: Ira sees no Exports entry anywhere on Home, and the organizer address renders the page's refusal sentence with no export controls on it.
...
- Screenshots of both export surfaces showing the stated scope, excluded fields, and columns, and of Dana's Home and Olive's Home showing which Exports entry each is offered.
- Screenshot of the Gate department Exports page offering no export, and of the offline state in step 32.
...
- Record any export surface offering an export the actor cannot run, whether it is offered as a disabled control or as one that fails when clicked.
