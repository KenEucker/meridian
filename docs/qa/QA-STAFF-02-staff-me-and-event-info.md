...
- If the connection indicator reads Connected while the node is not answering, stop testing and file a blocking issue against UI contract section 16.1A. Stop the node and reload to check: the indicator must go red and say "No node reachable", and Settings → Device diagnostics must agree with its own Server health line. A green indicator beside a failed health probe is the defect this check exists for, and everything downstream of it is being read on a page that cannot be believed.
