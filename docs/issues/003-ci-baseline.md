...
- Pushes to `production` run the same process validators and lightweight Composer/Node checks.
- Pushes to `production` upload traceability, QA docs, and process summary artifacts.
...
GitHub Actions are advisory until `production` branch protection requires passing checks.
