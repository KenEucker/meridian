...
The surface asks the node whether it may render, because only the node knows: it is not served by an on-site node or by a deployable node locked to an event (PUBLIC-006), and a client holding nothing on such a node goes to sign in instead — from `/platform` too, which sends an unavailable visitor to the root rather than rendering a blank page. The development node is the one exemption from the lock half (M20): the seeded scenario always names an event on it so field surfaces resolve, its event context is scaffolding rather than a deployment, and every deployable role keeps the refusal. It carries Meridian identity and resolves no organization branding profile (PUBLIC-001, BRAND-003) — the one public surface here that does not, since `public.participate` deliberately wears the organization's identity and this page has no organization in the request at all.

Milestone 20 made the surface the platform landing page (PUBLIC-007 through PUBLIC-009). The feature tour renders one section per entry of the code-defined catalogue in `marketingTour.ts` — a title, a short description of what the feature does for an organization, and one or two labelled perspectives: the lead or organizer side running the feature, and, wherever the product has a real second surface, the staff member or applicant side living it, each with its own micro-feature list and its own committed screenshot of the seeded Northwood scenario with descriptive alt text (PUBLIC-008; the capture and review process is `docs/process/marketing-screenshot-recapture.md`). A two-sided feature offers an accessible switch that swaps the micro-features and the screenshot together; a one-sided feature offers no switch, because a perspective is never mocked up. Each screenshot opens larger in an in-page viewer — a modal dialog over the same committed asset, closed by Escape, backdrop, or its close control, with focus returned to the screenshot that opened it — because a screenshot sized for a column is too small to actually read an interface in. The three platform offerings are described after the tour, descriptively only: no price, no payment or billing control, and no signup path, because organization creation stays a God Mode action (PUBLIC-009, PUBLIC-004). Both the tour and the offerings end at an in-page link to the organization interest form the surface already carried, so reading and acting stay on one page.
...
The Kiosk shell shows the active user and the controls for their session in its header, outside anything a screen can collapse or scroll away, because "the active user is shown prominently at all times" (technical spec 13.3) is not a property a screen can be trusted to preserve. The header's user control carries the name — it already does so in every mode — and the Kiosk adds beside it the control that ends the session and the warning before the inactivity timeout with a control that continues it. Both are absent while the workstation is locked.

This was a separate bar above the routed surface until M18.66, which is why the wording changed: a bar naming the active user sat directly beneath a header already naming them, so the same fact appeared twice in one piece of chrome, and the control that ends a session sat in the content area where it read as part of whichever screen was open. One name, in the place every other mode puts it, and the session controls beside it.
