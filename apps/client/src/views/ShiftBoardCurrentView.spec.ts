...
describe("Current Shift Board roster surface (M10.1 through M10.9)", () => {
  ...
  it("shows checked-out equipment and checks equipment in and out", async () => {
    ...
    const addAndCheckoutForm = updatedEquipmentSection.findAll("form")[1];
    await addAndCheckoutForm.find("input").setValue("Radio 14");
    await addAndCheckoutForm.findAll("input")[1].setValue("RDO-14");
    await addAndCheckoutForm
      .find("select")
      .setValue("aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaa2");
    await addAndCheckoutForm.trigger("submit");

    expect(wrapper.get('[aria-label="Equipment workflow status"]').text()).toBe(
      "Radio 14 added and checked out to Vera Staff.",
    );
    expect(wrapper.get('[aria-labelledby="equipment-heading"]').text()).toContain(
      "Radio 14",
    );

    const returnForm = wrapper
      .get('[aria-labelledby="equipment-heading"]')
      .findAll("form")[2];
    await returnForm.findAll("select")[0].setValue(
  ...
  it("checks multiple equipment items back in for one staff member", async () => {
    ...
    await checkoutForm.findAll("select")[0].setValue("equipment-safety-vest");
    await checkoutForm
      .findAll("select")[1]
      .setValue("aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaa2");
    await checkoutForm.trigger("submit");

    const addAndCheckoutForm = wrapper
      .get('[aria-labelledby="equipment-heading"]')
      .findAll("form")[1];
    await addAndCheckoutForm.findAll("input")[0].setValue("Radio 14");
    await addAndCheckoutForm.findAll("input")[1].setValue("RDO-14");
    await addAndCheckoutForm
      .find("select")
      .setValue("aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaa2");
    ...
    const returnForm = wrapper
      .get('[aria-labelledby="equipment-heading"]')
      .findAll("form")[2];
    expect(returnForm.text()).toContain("Safety Vest");
    expect(returnForm.text()).toContain("Radio 14");

    for (const checkbox of returnForm.findAll('input[type="checkbox"]')) {
    ...
    expect(wrapper.get('[aria-label="Equipment workflow status"]').text()).toBe(
      "2 items checked in as Returned.",
    );
    const checkedOutRows = wrapper
      .get('[aria-labelledby="equipment-heading"]')
      .findAll(".shift-board__checked-item")
