...
class ClientAppRouteTest extends TestCase
{
    ...
    public function test_a_dev_server_holding_only_ipv6_loopback_is_still_found(): void
    {
        // Vite may bind `::1` alone, and which family it holds is not this
        // server's choice. Probing the IPv4 form of `localhost` and stopping
        // there reports a running dev server as absent, which serves a stale
        // build while hot reload appears broken — quieter, and worse, than the
        // blank page the fallback exists to fix.
        $socket = @stream_socket_server('tcp://[::1]:0', $errorNumber, $errorMessage);

        if (! is_resource($socket)) {
            $this->markTestSkipped('This host has no IPv6 loopback to bind: '.$errorMessage);
        }

        $this->devServerSocket = $socket;
        $address = stream_socket_get_name($socket, false);
        $port = (int) substr($address, strrpos($address, ':') + 1);

        $this->installClientDist('<!doctype html><html><head></head><body>Built client</body></html>');
        config()->set('meridian.client.use_dev_server', true);
        config()->set('meridian.client.dev_server_url', 'http://localhost:'.$port);
        ...
        $response->assertDontSee('Built client', false);
    }
